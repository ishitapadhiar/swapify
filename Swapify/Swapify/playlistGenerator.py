...
class PlaylistGenerator:
    ...
    def generate_user_playlist(
        self, name, mood, friends=False, time=None, numSongs=None
    ):
        ...
        # If we are generating the playlist based on a time limit
            ...
            subset = [
                [False for i in range(time + 1)] for i in range(len(fullSongList) + 1)
            ]
                ...
                for j in range(1, time + 1):
                    ...
                    if j >= songLength:
                        if subset[i - 1][j][0]:
                            ...
                            subset[i][j] = (
                                subset[i - 1][j - songLength][0],
                                subset[i - 1][j - songLength][1].append(
                                    fullSongList[i - 1]
                                ),
                            )
                        else:
