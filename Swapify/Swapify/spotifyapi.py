...
class RandomSongGenerator():
	...
	def get_token(self):
		...
		client_token = base64.b64encode("{}:{}".format(self.CLIENT_ID, self.CLIENT_SECRET).encode('UTF-8')).decode('ascii')
		headers = {"Authorization": "Basic {}".format(client_token)}
		payload = {"grant_type": "client_credentials"}
		token_request = requests.post(self.SPOTIFY_TOKEN_URL, data=payload, headers=headers)
